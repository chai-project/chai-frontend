...
const MinimumPrice: React.FC<{periodState:any, energyPrice:any}> = ({periodState, energyPrice}) => {
    ...
    useEffect(()=>{
        switch(periodState) {
          case 'Today':
            // setMinPrice(Math.min(...energyPrice.averagePriceToday?.map((period:any) => period.rate)))
            break;
          case 'This week':
            // setMinPrice(Math.min(...energyPrice?.averagePriceThisWeek.map((period:any) => period.rate)))
            break;
          case 'This month':
            // setMinPrice(Math.min(...energyPrice?.averagePriceThisMonth.map((period:any) => period.rate)))
            break;
          default:
            setMinPrice(null)
        }
