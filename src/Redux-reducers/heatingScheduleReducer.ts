...
export const initializeHeatingSchedule = (label:String) => {
    return async (dispatch : Dispatch, getState:any) => {
        ...
        const heatingScheduleData:any = await services.getHeatingScheduleData(label);

        if(heatingScheduleData.error){
            dispatch({
                type:"SET_HEATING_SCHEDULE_DATA",
                data: {error: heatingScheduleData.error }
            })
        }else{                 
            let schedule: any[] = [];
            const profileLabels =[
            ...
            heatingScheduleData.forEach((day:any, index:any, self:any)=>{ //define types later
                ...
                for (const key in day.schedule) {
                    if(i === 0 && key !== "0"){
                        const emptyTimeslot = {
                            id: i,
                            profileName: Object.values(self[index-1].schedule).pop() === "1" ? profileLabels[1-1].name : Object.values(self[index-1].schedule).pop() === "2" ? profileLabels[2-1].name :Object.values(self[index-1].schedule).pop() === "3" ? profileLabels[3-1].name : Object.values(self[index-1].schedule).pop() === "4" ? profileLabels[4-1].name : profileLabels[5-1].name, 
